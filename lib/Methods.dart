...
class Notes_TestingElement extends TestingElement {
  ...
  @override
  Future<void> init(Database db) async {
    List<Map<String, dynamic>> value = await db.rawQuery('SELECT method_name, method_time FROM notes WHERE time = $time');
    actualElement = await getTestingElementFromMethodTime(value[0]["method_name"], value[0]["method_time"], db);
  }
